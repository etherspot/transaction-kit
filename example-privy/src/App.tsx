...
const AppContent = () => {
    ...
    const initializeKit = async () => {
      try {
        ...
        if (!kit) {
          ...
          logMessage(`TransactionKit initialized with wallet: ${wallet.address}`);
        ...
        logMessage(`Error initializing TransactionKit: ${error.message || error}`);
      }
    };
  ...
  const createTransaction = useCallback(async () => {
    ...
    try {
      ...
      logMessage(`Current state after creation: ${JSON.stringify(state, null, 2)}`);
  ...
  const estimateTransaction = useCallback(async () => {
    ...
    try {
      ...
      try {
        const etherspotProvider = kit.getProvider();
        ...
        logMessage(`Provider has request: ${typeof (provider as any)?.request === 'function'}`);
      } catch (error: any) {
        logMessage(`ERROR getting provider before estimation: ${error.message}`);
      }
      ...
      if (result.isEstimatedSuccessfully) {
        ...
        try {
          ...
          logMessage(`ERROR getting provider after estimation: ${error.message}`);
        }
        logMessage(`=== END DEBUG ===`);
      } else {
  ...
  const sendTransaction = useCallback(async () => {
    ...
    try {
      ...
      try {
        ...
        logMessage(`Provider has request method: ${typeof freshProvider.request === 'function'}`);
      } catch (error: any) {
      ...
      try {
        const etherspotProvider = kit.getProvider();
        logMessage(`EtherspotProvider config: ${safeStringify(etherspotProvider.getConfig())}`);
      } catch (error: any) {
      ...
      if (result.isSentSuccessfully) {
        logMessage(`Transaction sent successfully! UserOp Hash: ${result.userOpHash}`);
      } else {
      ...
      try {
        ...
        logMessage(`ERROR getting provider after exception: ${providerError.message}`);
      }
      logMessage(`=== END DEBUG ===`);
    } finally {
      setIsLoading(false);
